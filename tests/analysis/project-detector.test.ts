...
describe('detectProject', () => {
  it('detects a React project', () => {
    const info = detectProject(path.join(FIXTURES, 'react-project'), makeSimpleLogger());
  ...
  it('detects an Angular project', () => {
    const info = detectProject(path.join(FIXTURES, 'angular-project'), makeSimpleLogger());
    expect(info.types).toContain('angular');
    expect(info.framework).toBe('angular');
  });

  it('detects a Chrome extension', () => {
    const info = detectProject(path.join(FIXTURES, 'chrome-extension'), makeSimpleLogger());
    expect(info.types).toContain('chrome-extension');
  });

  it('detects a monorepo', () => {
    const info = detectProject(path.join(FIXTURES, 'monorepo'), makeSimpleLogger());
  ...
  it('returns basic info for simple project', () => {
    const info = detectProject(path.join(FIXTURES, 'simple-project'), makeSimpleLogger());
